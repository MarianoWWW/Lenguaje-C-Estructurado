...
int main()
{
    ...
    else
    {
        ...
        fclose(arch);
        FILE * arch1=fopen("productos.csv","wt");
        if (arch1 == NULL)
        {
            printf("\n\nError al abrir el archivo csv en modo escritura.");
            system("pause");
            exit(1);
        }
        fprintf(arch1,"Codigo;Precio;Descripcion\n");
        for (i = 0 ; i < cant ; i++)
        {
            fprintf(arch1,"%d;%.2f;%s\n", (producto+i)->codigo, (producto+i)->precio, (producto+i)->descripcion);
        }
        fclose(arch1);
    }
